+++
date = "2018-08-11T17:31:52+08:00"
title = "Talust Jenkins"
categories = ["doc"]
tags = ["jenkins"]
toc = true
author = "younglinuxer"
...
#!/bin/bash
#程序构建 生成jar包在 /data/jenkins_home/workspace/spring-talust-auth/｛app｝/target 下面
export MAVEN_HOME=/data/app/maven-3.3.9
export PATH=$PATH:$MAVEN_HOME/bin
...
#循环部署项目
for i in $DEPLOY; do
	if [[ $i == talust-enterprises ]]; then
		#statement
		for app in `ls -l $HOME/talust-enterprises|grep ^d|awk '{print $9}'`; do
			#statements
			mkdir /data/jenkins_home/docker/$app -pv
			cp $HOME/talust-enterprises/$app/target/*.jar /data/jenkins_home/docker/$app/
	...
	elif [[ $i == talust-framework ]]; then
		#statements
		for app in `ls -l $HOME/talust-framework|grep ^d|awk '{print $9}'`; do
			#statements
			mkdir /data/jenkins_home/docker/$app -pv
			cp ${HOME}/talust-framework/$app/target/*.jar /data/jenkins_home/docker/$app/
			cp /data/jenkins_home/dockerfile/* /data/jenkins_home/docker/$app/
			sed -i "s@APP_NAME@$app@g" /data/jenkins_home/docker/$app/Dockerfile
		done
	elif [[ $i == talust-commons ]]; then
		#statements
		for app in `ls -l $HOME/talust-commons|grep ^d|awk '{print $9}'`; do
			#statements
			mkdir /data/jenkins_home/docker/$app -pv
			cp ${HOME}/talust-commons/$app/target/*.jar /data/jenkins_home/docker/$app/
			cp /data/jenkins_home/dockerfile/* /data/jenkins_home/docker/$app/
			sed -i "s@APP_NAME@$app@g" /data/jenkins_home/docker/$app/Dockerfile
		done
	else
...
npm config set registry http://dev.talust.com:18002/nexus/repository/npm
#npm install -d
npm install -g
npm update
npm config set unsafe-perm true
...
npm run build:test
# 更新源码目录
rm -fr /data/jenkins_home/talust-ui/app/enterprise/
mv /data/jenkins_home/workspace/ui-manager-enterprise/dist /data/jenkins_home/talust-ui/app/enterprise
...
#node 
npm编译程序 > 替换docker(/data/jenkins_home/talust-ui)对应的源码　> 编译docker并推送到harbor仓库
...
#!/bin/bash
# docker 编译并推送镜像
docker login -uyuan.yang -paA52419589 https://dev.talust.com:18443
for i in `ls /data/jenkins_home/docker`
do
	docker build -t dev.talust.com:18443/talust-v2/$i /data/jenkins_home/docker/$i/
    docker push dev.talust.com:18443/talust-v2/$i:latest
    docker tag dev.talust.com:18443/talust-v2/$i:latest  dev.talust.com:18443/talust-v2/$i:`date +%F`
    docker push dev.talust.com:18443/talust-v2/$i:`date +%F`
done 
...
s.connect(hostname='172.17.234.25',port=65322,username='root',password='Talust.666test')
...
docker-compose -f /data/app/talust-v2/docker-compose-framework.yml pull
docker-compose -f /data/app/talust-v2/docker-compose-framework.yml up -d --force-recreate --no-deps ui-manager
# 一组项目更新 全部重建
docker-compose -f /data/app/talust-v2/docker-compose-commons.yml pull
docker-compose -f /data/app/talust-v2/docker-compose-commons.yml down
docker-compose -f /data/app/talust-v2/docker-compose-commons.yml up -d
